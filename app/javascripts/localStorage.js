...
define([
  ...
  'backbone/collections/stage'
	...
	if (checkStorageSupport()) {
		...
		if (!localStorage.getItem('UID')) {				// If a session does not already exist
	...
	// -> We'll remove the shouldSucceed param for production, this is just for debugging purposes
	function clickStorageButton(shouldSucceed) {
		console.log("You clicked the set storage button");

		//save interaction to action field
		...
		//save window.csf.stageCollection to localStorage
