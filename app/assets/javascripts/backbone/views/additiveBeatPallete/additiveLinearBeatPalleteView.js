// Filename: views/AdditiveBeatPallete/additiveLinearBeatPallete.js
/*
...
define([
  ...
  'app/dispatch'
], function($, _, Backbone, additiveBeatPallete, linearBeatPalleteTemplate, dispatch){
  var additiveLinearBeatPalleteView = Backbone.View.extend({
    ...
    initialize: function(){
      ...
      this.collection = additiveBeatPallete;
      this.collection = additiveBeatPallete.add({ width: 1, label: '4/4'});
      this.collection = additiveBeatPallete.add({ width: 0.5, label: '2/4'});
      this.collection = additiveBeatPallete.add({ width: 0.25, label: '1/4'});
      // this.collection = additiveBeatPallete.add({ width: 0.125, label: '1/8'});
    ...
    render: function(){
      ...
      console.log(data);
      var compiledTemplate = _.template( linearBeatPalleteTemplate, data );
    ...
    */
    reconfigure: function(signature) {
      ...
      this.collection = additiveBeatPallete.add({ width: 1, label: signature + '/' + signature});

      if (signature == 2) {
        this.collection = additiveBeatPallete.add({ width: 0.5, label: '1/2'});
        // this.collection = additiveBeatPallete.add({ width: 0.25, label: '1/4'});
      } else {
        for (var i=2; i <= Math.ceil(signature/2); i++) { 
          if (signature % i == 0) {
            ...
            // this.collection = additiveBeatPallete.add({ width: 1/(2*signature), label: '1/' + signature*2});
            break;
          }
          else {
            this.collection = additiveBeatPallete.add({ width: 1/signature, label: '1/' + signature});
            // this.collection = additiveBeatPallete.add({ width: 1/(2*signature), label: '1/' + signature*2});
            break;
          }
        }
    ...
    */
    divider: function(numerator, denominator, divisor) {
      if (numerator % divisor != 0) {
        if (numerator != 1) {
          this.collection = additiveBeatPallete.add({ width: numerator/denominator, label: numerator + '/' + denominator});
        }
        this.collection = additiveBeatPallete.add({ width: 1/denominator, label: '1/' + denominator});
